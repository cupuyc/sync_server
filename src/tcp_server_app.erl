...
start(_Type, _Args) ->
    ...
    supervisor:start_link({local, ?MODULE}, ?MODULE, [ListenPort, tcp_user_fsm]).
    ...
    {ok,
        {_SupFlags = {one_for_one, ?MAX_RESTART, ?MAX_TIME},
            [
			  ...
			  {   map_server_sup,                          % Id       = internal id
                  {map_server,start_link,[]}, % StartFun = {M, F, A}
                  permanent,                               % Restart  = permanent | transient | temporary
                  2000,                                    % Shutdown = brutal_kill | int() >= 0 | infinity
                  worker,                                  % Type     = worker | supervisor
                  [map_server]                             % Modules  = [Module] | dynamic
              }
            ]
        }
    };
